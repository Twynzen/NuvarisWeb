...
export class MagmaCoreSystem implements PlanetLayer {
  ...
  private createGlowLayers(): void {
    // MUCH REDUCED glow - only 2 very subtle layers
    // The core should NOT look like a sun
    const glowConfigs = [
      { radius: this.config.coreRadius * 1.08, color: 0xff4400, opacity: 0.15, falloff: 3.0 },
      { radius: this.config.coreRadius * 1.2, color: 0x881100, opacity: 0.08, falloff: 2.5 },
    ];
    ...
    glowConfigs.forEach(config => {
      ...
      const material = new THREE.ShaderMaterial({
        uniforms: {
          ...
          intensity: { value: config.opacity },  // No multiplier
