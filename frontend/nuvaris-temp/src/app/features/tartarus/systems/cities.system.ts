...
class SteampunkBuilding {
  ...
  private buildTower(): void {
    ...
    // Top light - BRIGHTER beacon
    this.addLight(new THREE.Vector3(0, 3.7, 0), 0xffdd88, 1.2, 5);
  }
  ...
  private buildDome(): void {
    ...
    // Interior light - BRIGHTER for visibility
    this.addLight(new THREE.Vector3(0, 0.5, 0), 0xffdd88, 1.8, 4.0);
  }
  ...
  private addWindows(radius: number, yPos: number, count: number): void {
    const windowGeometry = new THREE.PlaneGeometry(0.08, 0.12);  // Slightly larger
    // MUCH BRIGHTER windows for visibility
    const windowMaterial = new THREE.MeshBasicMaterial({
      color: 0xffee88,      // Brighter yellow-white
      transparent: true,
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Add small point light at each window for glow effect
      if (i % 2 === 0) {
        const windowLight = new THREE.PointLight(0xffcc66, 0.3, 1.5);
  ...
  private addFactoryWindows(): void {
    const windowGeometry = new THREE.PlaneGeometry(0.18, 0.14);  // Larger windows
    // MUCH BRIGHTER factory windows
    const windowMaterial = new THREE.MeshBasicMaterial({
      color: 0xffee99,      // Bright warm yellow
      transparent: true,
      opacity: 1.0,         // Full opacity
    });

    // Front windows with lights
    for (let i = 0; i < 4; i++) {
      const window = new THREE.Mesh(windowGeometry, windowMaterial.clone());
      const pos = new THREE.Vector3(-0.4 + i * 0.27, 0.5, 0.36);
      window.position.copy(pos);
      this.group.add(window);

      // Add light to each factory window
      const windowLight = new THREE.PointLight(0xffcc66, 0.4, 2.0);
    ...
    for (let i = 0; i < 2; i++) {
      const window = new THREE.Mesh(windowGeometry, windowMaterial.clone());
      const pos = new THREE.Vector3(0.61, 0.5, -0.15 + i * 0.3);
      window.position.copy(pos);
      window.rotation.y = Math.PI / 2;
      this.group.add(window);

      const windowLight = new THREE.PointLight(0xffcc66, 0.35, 1.8);
      windowLight.position.copy(pos);
      this.lights.push(windowLight);
      this.group.add(windowLight);
    }
  ...
  private addSmallWindow(position: THREE.Vector3): void {
    const windowGeometry = new THREE.PlaneGeometry(0.1, 0.1);  // Slightly larger
    // BRIGHT small windows
    const windowMaterial = new THREE.MeshBasicMaterial({
      color: 0xffee88,      // Bright yellow-white
      transparent: true,
      opacity: 1.0,
    });
    ...
    // Add small light for window glow
    const windowLight = new THREE.PointLight(0xffcc66, 0.25, 1.2);
