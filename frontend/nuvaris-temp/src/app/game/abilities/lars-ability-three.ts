...
export class LarsAbilityThree implements CharacterAbilityThree {
    ...
    // ========== SONIC SCREAM EFFECT (Grito Sónico) ==========
    /**
     * Crea efecto visual de GRITO SÓNICO
     * - Tubo cilíndrico semi-transparente desde Lars al enemigo
     * - Ondas sónicas que se propagan (anillos sutiles)
    ...
    public createMentalAttackEffect(
        ...
        // Punto medio entre Lars y el enemigo
        const midPoint = new THREE.Vector3().addVectors(playerPos, enemyPos).multiplyScalar(0.5);
        midPoint.y += 1.2; // Altura del grito (nivel de cabeza)

        // 1. TUBO CILÍNDRICO PRINCIPAL (Grito Sónico)
        const tubeRadius = 0.25; // Radio sutil
        const tubeGeometry = new THREE.CylinderGeometry(
            tubeRadius,      // Radio superior
            tubeRadius * 0.6, // Radio inferior (ligeramente cónico)
            distance,        // Longitud = distancia al enemigo
            12,              // Segmentos radiales (minimalista)
            1,               // Segmentos de altura
            true             // Abierto en los extremos
        );

        const tubeMaterial = new THREE.MeshBasicMaterial({
            color: 0x88ccff, // Azul claro sónico
            transparent: true,
            opacity: 0.3,
            side: THREE.DoubleSide
        });

        const sonicTube = new THREE.Mesh(tubeGeometry, tubeMaterial);

        // Posicionar y rotar el tubo para apuntar al enemigo
        sonicTube.position.copy(midPoint);

        // Calcular rotación para alinear el cilindro (eje Y) con la dirección
        const up = new THREE.Vector3(0, 1, 0);
        const quaternion = new THREE.Quaternion().setFromUnitVectors(up, direction);
        sonicTube.quaternion.copy(quaternion);

        scene.add(sonicTube);

        // 2. ONDAS SÓNICAS (anillos que se propagan)
        ...
        for (let i = 0; i < waveCount; i++) {
            const ringGeometry = new THREE.RingGeometry(
                tubeRadius * 0.8,  // Radio interno
                tubeRadius * 1.2,  // Radio externo
                16                 // Segmentos
            );

            const ringMaterial = new THREE.MeshBasicMaterial({
                color: 0xaaddff, // Azul más claro
                transparent: true,
                opacity: 0.4,
                side: THREE.DoubleSide
            });
            ...
            // Posición inicial: desde Lars hacia el enemigo
            const t = (i + 1) / (waveCount + 1);
            ring.position.lerpVectors(playerPos, enemyPos, t);
            ring.position.y += 1.2;

            // Orientar el anillo perpendicular a la dirección
            ring.quaternion.copy(quaternion);

            // Guardar progreso de onda
            (ring as any).waveProgress = t;
            (ring as any).initialScale = 1;
        ...
        // 3. ANIMACIÓN
        let frame = 0;
        const maxFrames = 25; // ~0.7 segundos a 35ms por frame

        const animInterval = setInterval(() => {
            frame++;
            const progress = frame / maxFrames;

            // Expandir el tubo ligeramente (efecto de onda)
            const pulseScale = 1 + Math.sin(frame * 0.5) * 0.1;
            sonicTube.scale.set(pulseScale, 1, pulseScale);

            // Fade out del tubo
            tubeMaterial.opacity = 0.3 * (1 - progress);

            // Animar ondas: expandir y mover hacia el enemigo
            for (let i = 0; i < waveRings.length; i++) {
                const ring = waveRings[i];
                const ringMat = waveMaterials[i];
                ...
                const waveT = Math.min(1, (ring as any).waveProgress + progress * 0.3);
                ring.position.lerpVectors(playerPos, enemyPos, waveT);
                ring.position.y += 1.2;

                // Fade out
                ringMat.opacity = 0.4 * (1 - progress);
            }

            // Cleanup al final
            if (frame >= maxFrames) {
                scene.remove(sonicTube);
