...
export class PunchWeapon extends Weapon {
    ...
    protected override fire(target: any): void {
        ...
        // Delay the attack to sync with animation (800ms like other characters)
        this.scene.time.delayedCall(800, () => {
            const angle = Phaser.Math.Angle.Between(this.player.x, this.player.y, target.x, target.y);

            // 1. Visual Effect: sprite appearing in front
            const offset = 60;
            ...
            const punchVisual = new VisualComponent(this.scene, {
                type: 'sprite',
                texture: 'arcadio-projectile-1', // Use first frame as static
                width: 80,
                height: 80
            });
            ...
            enemies.forEach(enemy => {
                ...
                if (dist <= hitRadius) {
                    ...
                    if (enemy.body) {
                        ...
                        // Use applyKnockback if available
