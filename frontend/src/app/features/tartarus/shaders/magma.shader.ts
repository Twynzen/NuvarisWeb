...
export const MagmaShader = {
  ...
  vertexShader: `
    ...
    void main() {
      ...
      // Deform surface to simulate lava bubbles
      vec3 pos = position;
      float noise = snoise(pos * noiseScale * 0.5 + time * 0.1);
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      // Additional flowing hotspots
      float hotspot2 = snoise(vPosition * 0.8 + vec3(0.0, time * 0.15, time * 0.1));
      hotspot2 = pow(max(hotspot2, 0.0), 3.0) * 0.8;

      // Mix colors based on "temperature"
      vec3 color = mix(coolColor, baseColor, combinedNoise);
      color = mix(color, hotColor, hotspot + hotspot2);

      // Add glow in hot areas
      float glow = hotspot * 0.5 + combinedNoise * 0.3 + hotspot2 * 0.4;
      color += vec3(glow * 0.5, glow * 0.2, 0.0);

      // Fresnel for bright edges
      vec3 viewDir = normalize(cameraPosition - vWorldPosition);
      float fresnel = pow(1.0 - max(dot(normalize(vNormal), viewDir), 0.0), 3.0);
      color += hotColor * fresnel * 0.4;
      ...
      // Dark veins/cracks
      float cracks = fbm(vPosition * 8.0 + time * 0.05);
      cracks = smoothstep(0.4, 0.6, cracks);
      color = mix(color, coolColor * 0.3, cracks * 0.3);
